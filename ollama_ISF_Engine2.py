...
Enhanced neuro-symbolic framework: Runs locally with Ollama for generation (using acereason-nemotron-abliterated:7b for advanced reasoning), handles file uploads (text/PDF), and produces detailed propositions/derivations.

Local Setup:
- Install: pip install ollama transformers torch streamlit networkx matplotlib pdfplumber
- Install Ollama: Follow https://ollama.com/ instructions, then pull the model: ollama pull huihui_ai/acereason-nemotron-abliterated:7b
- (Optional alternatives: ollama pull huihui_ai/phi4-reasoning-abliterated or others listed)
- Run: streamlit run neuro_symbolic_llm.py


Changes Made
I selected huihui_ai/acereason-nemotron-abliterated:7b as the primary model for text generation in this update, as it aligns with your preference (it's based on Nemotron variants optimized for reasoning tasks, which fits the neuro-symbolic framework's emphasis on logical inference). I defaulted to this in the code but kept flexibility for alternatives like phi4-reasoning-abliterated (you can pass it as an argument to NeuroSymbolicLLM(ollama_model='huihui_ai/phi4-reasoning-abliterated') if needed). The other options (jan-nano-abliterated:latest and qwen3-abliterated:4b) are compatible via Ollama but not defaulted, as Nemotron provides a strong balance of reasoning capability and efficiency for this use case.
...
Docstring: Updated to specify Ollama integration and the chosen model (huihui_ai/acereason-nemotron-abliterated:7b). Added setup notes for Ollama installation and model pulling. Mentioned alternatives from your list for easy switching.
Imports:
...
Switched from self.generator(prompt, ...)[0]['generated_text'] (transformers) to ollama.generate(model='huihui_ai/acereason-nemotron-abliterated:7b', prompt=prompt)['response'].
...
Initialization:
Added ollama_model parameter with default 'huihui_ai/acereason-nemotron-abliterated:7b'.
...
Pull the Model:
In a terminal: ollama pull huihui_ai/acereason-nemotron-abliterated:7b
This downloads the model (~2-4GB depending on quantization).
Alternatives:
For your favorite: ollama pull jan-nano-abliterated:latest
Others: ollama pull huihui_ai/phi4-reasoning-abliterated or ollama pull qwen3-abliterated:4b (then update the default in code or pass as arg).
Note: If using alternatives, ensure they support reasoning prompts well; Nemotron was chosen for its ablation optimized for logic tasks.
Hardware Requirements:
CPU/GPU: Works on CPU, but GPU (e.g., NVIDIA with CUDA) speeds up embeddings if torch detects it.
RAM: At least 8GB (model-dependent; Nemotron is efficient).
Run the App:
...
def unify_formula(f1: LogicalFormula, f2: LogicalFormula, subst: Optional[Dict[Var, Union[str, Var]]] = None) -> Optional[Dict[Var, Union[str, Var]]]:
    if subst is None:
        subst = {}
    ...
    for s1, s2 in zip(f1.subformulas, f2.subformulas):
        subst = unify_formula(s1, s2, subst)
        if subst is None:
            return None
...
class SymbolicAbstractionEngine:
    def __init__(self, embedder: pipeline, ollama_model: str):
    ...
    def extract_entities_and_relations(self, text: str) -> Dict[str, List[str]]:
        ...
        try:
            response = ollama.generate(model=self.ollama_model, prompt=prompt)['response']
...
class SymbolicInferenceEngine:
    ...
    def check_consistency(self) -> Tuple[bool, List[str]]:
        ...
        try:
            nx.find_cycle(self.kb.ontology_graph, orientation='original')
...
class NeuroSymbolicLLM:
    def __init__(self, ollama_model='huihui_ai/acereason-nemotron-abliterated:7b'):
        ...
        self.abstraction_engine = SymbolicAbstractionEngine(self.embedder, self.ollama_model)
    ...
    def process_input(self, input_text: str) -> Dict[str, Any]:
        ...
        refinement_round = 0
        max_refinements = 3
        while not is_consistent and refinement_round < max_refinements:
...
def main():
    ...
    st.caption("Runs locally with Ollama (acereason-nemotron-abliterated:7b), handles files, detailed propositions/derivations")
